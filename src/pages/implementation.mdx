...
### HYPERVSN — Entry Teaser / Satellite

Rotating LED fan arrays in a Glass Box. Loud without the box, limited facial nuance, but perfect as a lure near the entrance. Loop a 15–20 s "Oracle is speaking inside" piece or a procedural title (Veo 3 / TouchDesigner / C4D spline vibes). Keep the main Oracle on PG/light-field.

Ask them: live ingest (NDI/WebRTC?), latency, API hooks. If "no", treat it as pre-render only.
...
<div class="relative my-8">
  <div class="aspect-video rounded-lg overflow-hidden shadow-lg bg-black">
    <iframe 
      id="teaserVideo"
      src="https://www.youtube.com/embed/H1MRjgqqJjI?autoplay=1&mute=1&loop=1&playlist=H1MRjgqqJjI&controls=0&showinfo=0&rel=0&iv_load_policy=3&modestbranding=1&playsinline=1&enablejsapi=1"
      class="w-full h-full"
      style="border: none;"
  ...
  <button 
    id="teaserSoundToggle" 
    class="absolute top-2 right-2 bg-brand-teal hover:bg-brand-teal/80 text-white p-3 rounded-full transition-all duration-300 shadow-lg border-2 border-white/30"
    title="Toggle Sound">
    <svg id="teaserSoundIcon" class="w-5 h-5" fill="white" stroke="none" viewBox="0 0 24 24">
      <path id="teaserSoundPath" d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"></path>
      <path id="teaserMutePath" d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" class="opacity-0"></path>
    </svg>
  </button>
...
<script>
  let teaserMuted = true;
  let teaserPlayer;
  
  // Load YouTube API for teaser
  ...
  window.onYouTubeIframeAPIReady = function() {
    teaserPlayer = new YT.Player('teaserVideo', {
      events: {
        'onReady': onTeaserPlayerReady
      }
    });
  };
  
  function onTeaserPlayerReady(event) {
    const soundToggle = document.getElementById('teaserSoundToggle');
    const soundPath = document.getElementById('teaserSoundPath');
    const mutePath = document.getElementById('teaserMutePath');
    
    soundToggle.addEventListener('click', function() {
      if (teaserMuted) {
        teaserPlayer.unMute();
        teaserPlayer.setVolume(30);
